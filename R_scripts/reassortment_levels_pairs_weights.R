...
#save(pop.list, file = "final_project/run_043024/pop_list.RData")
...
w.calc <- function(gen){
  w <- sum(c((fit.vec[1] * (gen[1,])* (1-abs(gen[1,] - gen[2,]))) , #fitness of A
     (fit.vec[2] * (gen[2,]) * (1-abs(gen[1,] - gen[2,]))) , #fitness of B
      (fit.vec[3] * (gen[3,])))) #fitness of C
  return(w)
}
